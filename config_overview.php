...
if (isset($databases['Name'][0]) && $databases['Name'][0] > '')
{
	...
	else
	{
		...
		$aus['db'].='<tr class="thead"><th>' . $lang['L_DB'] . '</th><th>' .$lang['L_Multidump']. '<br><span class="ssmall">(<a href="javascript:SelectMD(true,' . count($databases['Name']) . ')" class="small">' . $lang['L_ALL'] . '</a>&nbsp;<a href="javascript:SelectMD(false,' . count($databases['Name']) . ')" class="small">' . $lang['L_NONE'] . '</a>)</span></th>';
		$aus['db'].='<th>' . Help($lang['L_HELP_PRAEFIX'],"conf2") . $lang['L_PRAEFIX'] . '</th><th>' . Help($lang['L_HELP_COMMANDS'],"",11) . 'Dump执行前的命令</th><th>' . Help($lang['L_HELP_COMMANDS'],"",11) . 'Dump执行后的命令</th><th>' . $lang['L_SQL_BEFEHLE'] . '</th></tr>';
